...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::ForceWithdraw {} => execute_force_withdraw(deps, env, info),
    }
}
...
pub fn is_inactive(start_timestamp: Timestamp, current_timestamp: Timestamp) -> bool {
    if !is_kickstarted(start_timestamp) {
        return true;
    }
...
pub fn is_kickstarted(start_timestamp: Timestamp) -> bool {
...
pub fn execute_withdraw(
    ...
    if is_inactive(config.schedule_start, env.block.time) {
    ...
    Ok(Response::new().add_message(send_request))
}
...
pub fn update_last_withdraw_to(
    ...
    Ok(SHAREHOLDERS.update(
        store,
        addr,
        |item: Option<ShareholderInfo>| -> StdResult<ShareholderInfo> {
            let mut i = item.ok_or(StdError::GenericErr {
                msg: String::from("unable"),
            })?;
...
pub fn calculate_weight_sum(deps: Deps) -> StdResult<Uint64> {
    Ok(SHAREHOLDERS
...
pub fn calculate_withdraw_amnt(deps: Deps, env: Env, addr: &Addr) -> StdResult<Uint128> {
    let config = CONFIG.load(deps.storage)?;
    ...
    let weight = (Uint128::from(member_info.weight), Uint128::from(weight_sum));

    let time_diff = Uint128::from(
        env.block
            .time
            .minus_seconds(member_info.last_withdraw_timestamp.seconds())
            .seconds(),
    );

    let time_weight = (time_diff, Uint128::from(config.vesting_period));

    Ok(balance
        .checked_mul_floor(time_weight)
...
pub fn query(deps: Deps, env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        ...
        QueryMsg::Members {} => Ok(to_binary(&query_members(deps, env)?)?),
        QueryMsg::Member { addr } => Ok(to_binary(&query_member(deps, env, addr)?)?),
    }
}

pub fn query_config(deps: Deps, _env: Env) -> StdResult<ContractConfig> {
    Ok(CONFIG.load(deps.storage)?)
}

pub fn query_members(deps: Deps, _env: Env) -> StdResult<QueryMembersResponse> {
    let addresses = SHAREHOLDERS
        .keys(deps.storage, None, None, Order::Ascending)
        .into_iter()
        .map(|item| item.unwrap())
        .collect::<Vec<_>>();
    let resp = QueryMembersResponse { members: addresses };
    Ok(resp)
}

pub fn query_member(deps: Deps, env: Env, addr: Addr) -> StdResult<QueryMemberResponse> {
